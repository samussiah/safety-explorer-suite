(function (global, factory) {
    ...
    function loadFiles(dataFiles, sdtm) {
    ...
    function loadSettings() {
        ...
        var location = this.config.chartSettings.location.path ? this.config.chartSettings.location.path + this.config.chartSettings.location.file + '.json' : 'library';

        //load the settings object
        if (location == 'library') {
            this.config.chartSettings.imported = this.settingsLibrary[this.config.chartSettings.location.file];
            ...
            d3.json(location, function (error, json) {
                if (error) {
                    ...
                    _this.config.chartSettings.imported = json;
                    _this.config.chartSettings.load = false;
                    _this.charts.init(_this);
                    _this.charts.renderers[0].render();
                }
            });
        }
    }
    ...
    function init(dataArray) {
            ...
            // load the csvs if requested
                            ...
                            if (this.config.chartSettings.load) loadSettings.call(this);else {
    ...
    function layout() {
        ...
        this.wrap = d3.select(this.element).append('div').attr('class', 'safety-explorer');
    ...
    function init$1() {
        ...
        var chartNav = this.nav.wrap.append('ul').attr('class', 'ses-nav ses-nav-tabs');

        var chartNavItems = chartNav.selectAll('li').data(this.charts.renderers).enter().append('li').classed('active', function (d, i) {
            return d.name === _this.config.initial_renderer.name;
        });
    ...
    function prepSettings() {
        ...
        this.charts.renderers.forEach(function (renderer) {
            var customMatch = _this.config.chartSettings.custom ? _this.config.chartSettings.custom.filter(function (f) {
                return f.renderer_name == renderer.name;
            }) : [];
            var importedMatch = _this.config.chartSettings.imported ? _this.config.chartSettings.imported.filter(function (f) {
                return f.renderer_name == renderer.name;
            }) : [];
    ...
    function init$2() {
        ...
        this.charts.renderers.forEach(function (renderer) {
            ...
            if (renderer.name == 'web-codebook') {
                renderer.settings.files = _this.data.map(function (d) {
                    d['Data'] = d.type;
                    d.Rows = d.raw.length;
                ...
                renderer.dataFile = _this.data.filter(function (d) {
            ...
            renderer.render = function () {
                if (renderer.sub) {
                    ...
                    _this.currentChart = window[renderer.main][renderer.sub](_this.element + ' .chartWrap', renderer.settings);
                } else {
                    _this.currentChart = window[renderer.main](_this.element + ' .chartWrap', renderer.settings);
                }
                _this.currentChart.key = renderer.name;
                _this.currentChart.renderer = renderer;

                if (renderer.dataFile) {
                    _this.currentChart.init(renderer.dataFile.raw.map(function (d) {
                        return Object.assign({}, d);
                    }));
                } else {
                    _this.currentChart.init();
                }

                //call the chartinit callback
                _this.events.onChartinit.call(_this);
            };
    ...
    function prepSettings$1() {
        ...
        this.config.renderers = this.config.renderers || this.charts.renderers.map(function (renderer) {
            return renderer.name;
        });
        this.config.custom_settings = this.config.custom_settings || defaultSettings.custom_settings;

        //only keep the selected renderers (or keep them all if none are specified)
        this.charts.renderers = this.charts.renderers.filter(function (d) {
            return _this.config.renderers.indexOf(d.name) > -1;
        }).sort(function (a, b) {
            return _this.config.renderers.indexOf(a.name) - _this.config.renderers.indexOf(b.name);
        });
        ...
        this.config.initial_renderer = this.charts.renderers.find(function (renderer) {
            return renderer.name === _this.config.initial_renderer;
        ...
        if (this.config.custom_settings) {
            this.config.custom_settings.forEach(function (custom_setting) {
                var thisRenderer = _this.charts.renderers.filter(function (renderer) {
        ...
        if (Array.isArray(this.config.filters)) {
            this.charts.renderers.forEach(function (module) {
                // Check for module-level filters.
                var moduleFilters = module.name === 'aeexplorer' ? module.settings.variables.filters || [] : module.settings.filters || [];

                // Add suite-level filters to module-level filters.
                _this.config.filters.forEach(function (filter) {
                    var moduleFilter = moduleFilters.find(function (moduleFilter) {
                        return moduleFilter.value_col === filter.value_col;
                    });
                    if (moduleFilter === undefined) moduleFilters.push(filter);
                });

                // Set module-level filters.
                if (module.name === 'aeexplorer') module.settings.variables.filters = moduleFilters;else module.settings.filters = moduleFilters;
            });
        }
    }
